...
public class BoardScript: MonoBehaviour {
  ...
  private void initializeTiles() {
    ...
    for (int i = -x; i < x; i++) {
      for (int j = -z; j < z; j++) {
        ...
        if (i == 0 && j == 0) {
          var manObject = instantiateObject(manPrefab, Vector3.zero);
          man = manObject.GetComponent<ManScript>();
          man.transform.parent = transform;
          man.transform.localPosition = Vector3.zero;
          man.name = string.Format("man");
          continue;
        }
  ...
  void Update() {
    ...
    if (updateType == TileUpdateType.DirectPerson) {
      man.seekPath(hit.point);
      return;
    }
  ...
  public static void adjustVertices(TileScript tile, float changeRate, TileUpdateType type, TileUpdateDirection direction, AstarPath path) {
    ...
    foreach (var neighbour in tile.neighbours) {
      ...
      path.UpdateGraphs(new Bounds(neighbour.transform.position,
  ...
  public void setDirected(bool active) {
    if (!active) {
      return;
    }
    updateType = TileUpdateType.DirectPerson;
  }
